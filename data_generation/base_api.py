...
class APICallPostprocessing:
    def __init__(
        ...
        self.api_text = ""  # API text, might be better to pass it in ### e.g What is the date today
...
### we have
    def filter_continuations(
        ...
        # Make sure we don't keep any tokens that are supposed to be [  ( <API> )
        ### Remove start tokens. Start tokens' probability is set to zero at `max_start_tokens`. should consider other tokens than <API>.
        ...
        ### top k positions that we can make API call
        ...
        for i, batch in enumerate(indices):
            for j, index in enumerate(batch):
                if values[i][j] < self.minimum_percentage:
                    continue
                # Get base output
                ### e.g "GPU is I cool" => "cool"
                ### compare base_outputs, i.e prediction (expensive) w/ labels (cool)
                ...
                ### L_i(epsilon)
                ### Essentialy Cross Entropy Loss function
                ### WHY : compare making api call w/ not making api call (??)
                base_loss = criterion(
...
### execute api
### trying to generate multiple text that has API call ~ no response (which means no API) / has API call but NO response.
    def add_api_calls(
        self,
        candidate: int,
        outputs: dict,
    ...
    def generate_continuations(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            ### `generated_texts` w/ wo/ api calls 
...
### pass generated text inside the model
### base : wo/ api calls, 
### test : w/ api calls
                ...
                for j in range(len(generated_texts)):
                    ...
                    ### Loss function within Threshold
                    if (
